...
describe("RA buyer", () => {
  const pageUrl =
    "https://platform.uat.impetusz0.de/auth/loginuvp/range-architecture";
  ...
  it("Test Case 3: Vendor verifies shared design and submits", () => {
    ...
    cy.get('table tbody tr', { timeout: 20000 }).should('contain.text', designname);
  ...
  it("Test Case 7: Vendor submits without making any changes to design", () => {
    ...
    cy.contains("32021182", { timeout: 10000 }).should("be.visible").click();
  ...
  it("Test Case 8: Vendor edits the HSN code and submits the design", () => {
    ...
    cy.intercept('**/oem-design/vendorview/get-submitted-design*').as('submittedSearch8');
    cy.get('input[placeholder="Search"]')
      .should("be.visible")
      .clear({ force: true })
      .type(designname, { force: true });
    cy.wait('@submittedSearch8', { timeout: 20000 });
    cy.get('table tbody tr', { timeout: 10000 }).should('contain.text', designname);
    cy.contains("p", "REWORK", { timeout: 15000 }).first().click({ force: true });
    cy.contains("div.n-button-content", "Edit", { timeout: 15000 }).click().wait(1000);
  ...
  it("Test Case 10: Vendor reworks on design 2 after cluster rework", () => {
    ...
    cy.contains("32021182", { timeout: 15000 }).click().wait(1000);
